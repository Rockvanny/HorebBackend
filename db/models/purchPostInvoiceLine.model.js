// models/purchPostInvoiceLine.model.js
const { Model, DataTypes, Sequelize } = require('sequelize');

const PURCHPOSTINVOICELINE_TABLE = 'purch_post_invoice_lines';

const purchPostInvoiceLineSchema = {
  // CLAVE COMPUESTA NORMALIZADA (Igual que en ofertas)
  codeDocument: {
    field: 'code_document',
    allowNull: false,
    primaryKey: true,
    type: DataTypes.STRING,
    references: { model: 'purch_post_invoices', key: 'code' },
    onUpdate: 'CASCADE',
    onDelete: 'CASCADE'
  },
...
class purchPostInvoiceLine extends Model {
  ...
  // Recalcular los totales de la cabecera de la factura de compra
  static async updateDocumentTotals(codeDocument, transaction) {
    const { purchPostInvoice } = this.sequelize.models;
    ...
    await purchPostInvoice.update({
      ...
      // quemando un número de serie por cada línea creada/borrada. Este
      // recálculo no necesita validar nada.
      validate: false
    });
  }
